      ...
      <div className="p-5 space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setDias(d => Math.min(60, d + 1))}
              className="w-9 h-9 rounded-xl bg-gray-100 font-bold text-gray-600 hover:bg-gray-200 transition-colors text-[18px]">
              +
            </button>
  ...
  return (
    <Modal title="Editar información personal" onClose={onClose}>
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Nombre *"><input className={inputCls()} value={form.firstName} onChange={e => setForm(f => ({ ...f, firstName: e.target.value }))} /></Field>
          <Field label="Apellidos *"><input className={inputCls()} value={form.lastName} onChange={e => setForm(f => ({ ...f, lastName: e.target.value }))} /></Field>
        </div>
        <Field label="Email"><input type="email" className={inputCls()} value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} /></Field>
        <div className="grid grid-cols-2 gap-3">
          <Field label="Teléfono"><input className={inputCls()} value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} /></Field>
          <Field label="Fecha alta"><input type="date" className={inputCls()} value={form.hireDate} onChange={e => setForm(f => ({ ...f, hireDate: e.target.value }))} /></Field>
        </div>
        <Field label="Notas internas"><textarea className={inputCls() + ' resize-none h-20'} value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="Observaciones sobre el empleado..." /></Field>
      </div>
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Tipo de contrato">
            <select className={inputCls()} value={form.contractType} onChange={e => setForm(f => ({ ...f, contractType: e.target.value }))}>
        ...
        <div>
          <Toggle
            ...
            value={form.hasRange}
            onChange={(v: boolean) => setForm(f => ({ ...f, hasRange: v }))}
          />
          {form.hasRange && (
              ...
              <Field label="Mínimo obligatorio">
                <div className="flex items-center gap-2">
                  <input type="number" min={1} max={48} className={inputCls()} value={form.minWeeklyHours}
                    onChange={e => setForm(f => ({ ...f, minWeeklyHours: +e.target.value }))} />
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <Field label="Descanso entre jornadas" hint="Convenio: 12h">
              <input type="number" min={10} max={24} className={inputCls()} value={form.minRestBetweenShifts}
                onChange={e => setForm(f => ({ ...f, minRestBetweenShifts: +e.target.value }))} />
            </Field>
            <Field label="Máx. días seguidos" hint="Recomendado: 6">
              <input type="number" min={1} max={7} className={inputCls()} value={form.maxConsecutiveDays}
                onChange={e => setForm(f => ({ ...f, maxConsecutiveDays: +e.target.value }))} />
            </Field>
          </div>
          <div className="mt-3">
            <Field label="Horas anuales máximas" hint="Convenio hostelería Tarragona: 1.791h">
              <input type="number" className={inputCls()} value={form.annualMaxHours}
                onChange={e => setForm(f => ({ ...f, annualMaxHours: +e.target.value }))} />
            </Field>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            <Toggle
              ...
              onChange={(v: boolean) => setForm(f => ({ ...f, preferContinuous: v }))}
            />
            <Toggle
              label="Acepta jornadas partidas"
              desc="Tramos 3–5h, ≥1.5h de descanso entre ellos, total ≤9h/día"
              value={form.allowSplit}
              onChange={(v: boolean) => setForm(f => ({ ...f, allowSplit: v }))}
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Coste/hora (€)" hint="Para estimaciones de coste">
            <input type="number" step="0.01" className={inputCls()} value={form.hourlyWage}
              onChange={e => setForm(f => ({ ...f, hourlyWage: e.target.value }))} placeholder="Ej: 12.50" />
          </Field>
          <Field label="Convenio colectivo">
            <input className={inputCls()} value={form.collectiveAgreement}
              onChange={e => setForm(f => ({ ...f, collectiveAgreement: e.target.value }))} />
          </Field>
        </div>

        {/* Vigencia */}
        <div className="grid grid-cols-2 gap-3">
          <Field label="Fecha inicio"><input type="date" className={inputCls()} value={form.startDate} onChange={e => setForm(f => ({ ...f, startDate: e.target.value }))} /></Field>
          <Field label="Fecha fin (opcional)"><input type="date" className={inputCls()} value={form.endDate} onChange={e => setForm(f => ({ ...f, endDate: e.target.value }))} /></Field>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <label className="flex items-center gap-2 mb-2 cursor-pointer">
            <input type="checkbox" checked={form.allDays} onChange={e => setForm(f => ({ ...f, allDays: e.target.checked }))}
