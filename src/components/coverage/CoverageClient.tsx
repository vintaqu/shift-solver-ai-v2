...
import {
  ...
  List, Info,
  AlertTriangle, Pencil, MoreHorizontal,
  LayoutGrid, Wand2,
} from 'lucide-react'
...
// ─── Template Bar ─────────────────────────────────────────────────────────────
const STATUS_CFG_TMPL: Record<string, { label: string; cls: string }> = {
...
function ActionsMenu({ onGenerate, onCopyDay, onAddSlot, onClearAll }: {
  onGenerate: () => void
  onCopyDay: () => void
  onAddSlot: () => void
  onClearAll: () => void
  ...
  const items = [
    { icon: <Sparkles size={14} />, label: 'Generar slots automáticamente', action: onGenerate, desc: 'Rellena todo el horario de una vez' },
    { icon: <Copy size={14} />, label: 'Copiar día a otro día', action: onCopyDay, desc: 'Duplica la configuración de un día' },
    { icon: <Plus size={14} />, label: 'Añadir slot individual', action: onAddSlot, desc: 'Crea un slot de 30 min concreto' },
    { divider: true },
    { icon: <Trash2 size={14} />, label: 'Borrar todos los slots', action: onClearAll, desc: 'Vacía la plantilla actual', danger: true },
  ]
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-2xl border border-gray-200 shadow-xl z-40 py-1.5 overflow-hidden">
          {items.map((item, i) =>
            item.divider ? (
              ...
              <button
                ...
                className={cn(
                  'w-full flex items-start gap-3 px-4 py-2.5 text-left hover:bg-gray-50 transition-colors',
                  item.danger && 'hover:bg-red-50'
                )}
              >
                <span className={cn('mt-0.5 flex-shrink-0', item.danger ? 'text-red-500' : 'text-gray-400')}>
                  {item.icon}
                </span>
                <div>
                  <div className={cn('text-[13px] font-medium', item.danger ? 'text-red-600' : 'text-gray-800')}>
                    {item.label}
                  </div>
...
// ─── Estado vacío ─────────────────────────────────────────────────────────────
function EmptyState({ onLoadTemplate, onGenerate }: { onLoadTemplate: () => void; onGenerate: () => void }) {
  return (
      ...
      <div className="max-w-md w-full text-center">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          <button
            ...
            className="flex flex-col items-center gap-3 p-5 rounded-2xl border-2 border-dashed border-gray-200 hover:border-indigo-300 hover:bg-indigo-50 transition-all group text-left"
          >
            <div className="w-10 h-10 rounded-xl bg-white border border-gray-200 group-hover:border-indigo-200 flex items-center justify-center text-xl shadow-sm">
              🍽️
            </div>
            <div>
          ...
          <button
            onClick={onGenerate}
            className="flex flex-col items-center gap-3 p-5 rounded-2xl border-2 border-indigo-600 bg-indigo-600 hover:bg-indigo-700 transition-all text-left"
          >
...
// ─── Modal: Cargar plantilla de ejemplo ──────────────────────────────────────
  ...
  const TEMPLATES = [
    {
      ...
      desc: 'Servicio de desayuno, mediodía y noche. Más demanda en fin de semana.',
      slots: '~60 slots · 7 días',
    },
    {
      id: 'cafe_desayunos',
      icon: '☕',
      name: 'Cafetería / Desayunos',
      desc: 'Solo mañanas 7:00–16:00. Brunch reforzado en fin de semana.',
    ...
    },
    {
      ...
      icon: '🌙',
      name: 'Bar nocturno',
      desc: 'Solo tardes y noches 18:00–00:00. Pico máximo viernes y sábado.',
    ...
    <Modal title="Cargar configuración de ejemplo" onClose={onClose}>
      <div className="space-y-4">
        <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-2 text-[12px] text-amber-800">
          ...
          <span>Esto <strong>reemplazará</strong> los slots actuales de esta plantilla con la configuración seleccionada.</span>
        </div>
        ...
        <ModalFooter
          ...
          onSave={() => startTransition(async () => {
            try {
              const result = await loadCoverageTemplate(
                locationId,
                organizationId,
                selected as any,
                templateId,
  ...
  return (
    <Modal title="Generar slots de cobertura" onClose={onClose}>
      <div className="space-y-5">
          ...
          <div className="grid grid-cols-7 gap-1">
            {DAYS_SHORT.map((d, i) => (
              <button
                ...
                onClick={() => toggleDay(i)}
                className={cn(
                  'py-2.5 rounded-xl text-[12px] font-bold transition-all',
        ...
        <Field label="Horario de apertura y cierre">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                type="time"
                value={form.closeTime === '00:00' ? '23:30' : form.closeTime}
        ...
        {/* Demanda */}
        <Field label="Personas por franja (valores por defecto)">
          <div className="flex gap-6">
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setForm(f => ({ ...f, defaultMin: f.defaultMin + 1 }))} className="w-8 h-8 rounded-lg bg-gray-100 font-bold hover:bg-gray-200 transition-colors">+</button>
              </div>
            </div>
            <div>
        ...
        {previewCount > 0 && (
          <div className="p-3 bg-indigo-50 border border-indigo-200 rounded-xl text-[12px] text-indigo-800">
            Se crearán <strong>{previewCount} slots</strong> de 30 min para {form.days.length} día{form.days.length !== 1 ? 's' : ''}.
            Podrás ajustar la demanda de cada franja individualmente después.
          </div>
        )}
          ...
          onSave={() => startTransition(async () => {
            try {
              ...
              for (const day of form.days) {
                const result = await generateSlotsForDay(
                  locationId,
                  organizationId,
                  day,
                  form.openTime,
        ...
        <Field label="Copiar desde">
          <div className="grid grid-cols-7 gap-1">
            {DAYS_SHORT.map((d, i) => (
              <button
                key={i}
                onClick={() => { if (availableFrom.includes(i)) setFromDay(i) }}
                disabled={!availableFrom.includes(i)}
                className={cn(
                  'py-2.5 rounded-xl text-[12px] font-bold transition-all',
          ...
          {availableFrom.length === 0 && (
            <p className="text-[11px] text-amber-600 mt-2">No hay ningún día con slots configurados.</p>
          )}
        </Field>
        ...
        <Field label="Copiar hacia">
          <div className="grid grid-cols-7 gap-1">
            {DAYS_SHORT.map((d, i) => (
              <button
                key={i}
                onClick={() => setToDay(i === fromDay ? null : i)}
                disabled={i === fromDay}
                className={cn(
                  'py-2.5 rounded-xl text-[12px] font-bold transition-all',
              ...
              >
                {d}
                {(dayRanges[i]?.count ?? 0) > 0 && i !== fromDay && (
                  <span className={cn('text-[9px] font-normal', toDay === i ? 'opacity-80' : 'text-amber-500')}>⚠️</span>
                )}
              </button>
            ))}
          </div>
...
// ─── Modal: Crear/Editar slot individual ─────────────────────────────────────
            ...
            {DAYS_SHORT.map((d, i) => (
              <button
                ...
                className={cn(
                  ...
                  form.dayOfWeek === i ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                )}
              >
                {d}
              </button>
          ...
          <Field label="Rol requerido (opcional)">
            <div className="flex flex-wrap gap-2">
              ...
              {roles.map((r: any) => (
                <button
                  key={r.id}
                  onClick={() => setForm(f => ({ ...f, laborRoleId: f.laborRoleId === r.id ? '' : r.id }))}
...
// ─── Vista Matriz ─────────────────────────────────────────────────────────────
function MatrixView({ slots, slotMap, visibleTimes, selectedDay, onEditSlot, onAddSlot }: any) {
            ...
            {visibleTimes.map((time: string) => (
              <tr key={time} className="group">
                <td className="sticky left-0 bg-white border-b border-r border-gray-100 px-3 py-0 z-10">
                  <span className={cn('text-[11px] font-mono font-semibold', time.endsWith(':00') ? 'text-gray-700' : 'text-gray-400')}>
                    {time}
                  </span>
                </td>
                {days.map((d: number) => {
                        ...
                        >
                          ...
                          <div className="pl-1 flex items-center justify-between">
                            ...
                            {slot.idealWorkers > slot.minWorkers && (
                              <span className="text-[10px] font-medium opacity-60" style={{ color: colors.text }}>/{slot.idealWorkers}</span>
                            )}
                          </div>
                          {slot.laborRole && (
                            <div className="pl-1">
                          ...
                          {/* Delete on hover */}
                          <button
                            className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-red-500 text-white opacity-0 group-hover/cell:opacity-100 transition-opacity flex items-center justify-center shadow-sm z-10"
        ...
        return (
          <div key={d}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[11px] text-gray-400">{daySlots.length} slots</span>
            </div>
            <div className="space-y-1.5">
              {daySlots.map((slot: Slot) => {
                ...
                return (
                  ...
                  >
                    <div className="w-1.5 h-8 rounded-full" style={{ backgroundColor: colors.bar }} />
                    <span className="text-[12px] font-mono text-gray-600 w-24">{slot.startTime} – {slot.endTime}</span>
...
// (se mantiene igual que antes — omitido por brevedad, incluir el existente)
...
export function CoverageClient({
  ...
  function handleClearAll() {
    ...
    startTransition(async () => {
      try {
        // Borrar uno a uno (no hay bulk delete action aún)
        for (const slot of initialSlots) {
    ...
    <div className="flex flex-col h-full" style={{ background: '#f5f6fa' }}>
      ...
      <div className="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          {hasSlots && (
            <div className="flex items-center gap-2">
              {/* Botón principal: añadir slot */}
              <button
                onClick={() => { setCreateDay(selectedDay ?? 0); setCreateTime('09:00'); setSlotModal('create') }}
              ...
              <ActionsMenu
                ...
                onAddSlot={() => { setCreateDay(selectedDay ?? 0); setCreateTime('09:00'); setSlotModal('create') }}
        ...
        {hasSlots && (
          <div className="flex items-center gap-6 mt-3 pt-3 border-t border-gray-100">
            ...
            {/* Vista y filtro día — al lado derecho */}
            <div className="ml-auto flex items-center gap-3">
              {/* Selector de día */}
              <div className="flex items-center gap-1 bg-gray-100 rounded-xl p-1">
                <button
                  onClick={() => setSelectedDay(null)}
                  className={cn('px-3 py-1 rounded-lg text-[12px] font-semibold transition-colors', selectedDay === null ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700')}
                >
                  Todos
                </button>
                {DAYS_SHORT.map((d, i) => (
              ...
              <div className="flex items-center gap-1 bg-gray-100 rounded-xl p-1">
                <button
                  ...
                  className={cn('flex items-center gap-1.5 px-3 py-1 rounded-lg text-[12px] font-semibold transition-colors', viewMode === 'matrix' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500')}
                >
                  <LayoutGrid size={13} /> Matriz
                </button>
                <button
                  onClick={() => setViewMode('list')}
                  className={cn('flex items-center gap-1.5 px-3 py-1 rounded-lg text-[12px] font-semibold transition-colors', viewMode === 'list' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500')}
                >
                  <List size={13} /> Lista
                </button>
              </div>
      ...
      <div className="flex-1 overflow-auto">
        {!hasSlots ? (
          ...
          <MatrixView
            ...
            onEditSlot={(slot: Slot) => setSlotModal(slot)}
            onAddSlot={openCreateFor}
          />
        ) : (
          <ListView
            slots={initialSlots}
            onEdit={(slot: Slot) => setSlotModal(slot)}
      ...
      {showGenerate && (
        <GenerateSlotsModal
          ...
          organizationId={organizationId}
          templateId={selectedTemplateId}
          defaultOpenTime={activeTemplate?.openingTime ?? '09:00'}
          defaultCloseTime={activeTemplate?.closingTime ?? '23:00'}
